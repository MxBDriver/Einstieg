    ...
    public static void main(String[] args) {
        ...
        for (zeilen = 0; zeilen <= zeiten.length;) {
            ...
            if (tag < zeiten[zeilen][0]) {
              ...
              // Zwei Buchungen
              // Wenn Aktuelle Zeile < Max. Anzahl Zeilen Array -1
            if ( < -1) {
                if (tag == zeiten[zeilen][0] && tag == zeiten[zeilen + 1][0]) {
                    // DANN
                    anwTag = zeiten[zeilen + 1][1] * 60 + zeiten[zeilen + 1][2] - (zeiten[zeilen][1] * 60 + zeiten[zeilen][2]);
                    anwMonat = anwTag + anwMonat;
                    System.out.println(tag + "-" + zeiten[zeilen][1] + " " + zeiten[zeilen][2] + " " + zeiten[zeilen + 1][1]+ " " + zeiten[zeilen + 1][2] + " Min: " + anwTag);
                    zeilen = zeilen + 2;
                } // ENDE ANSONSTEN WENN
            }  
            // Eine Buchung
                ...
                // DANN
                System.out.println(tag + "-" + zeiten[zeilen][1] + " " + zeiten[zeilen][2] + Integer.toString(-1) + "-1" + " " + anwTag + " Buchung fehlt");
                zeilen = zeilen + 1;
